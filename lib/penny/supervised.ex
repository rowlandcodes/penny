...
defmodule Penny.Supervised do
  ...
  defp stream_reduce({:cont, acc}, max, spawned, delivered, waiting, next, config)
    ...
    receive do
      ...
      {kind, {^monitor_ref, position}, _reason}
  ...
  defp stream_reduce({:cont, acc}, max, spawned, delivered, waiting, next, config) do
    try do
      ...
      {:suspended, [value], next} ->
        discriminate_value(value, spawned, waiting, config, acc, max, delivered, next)

      {_, [value]} ->
        discriminate_value(value, spawned, waiting, config, acc, max, delivered, :done)
  ...
  defp discriminate_value(value, spawned, waiting, config, acc, max, delivered, next) do
    case Penny.Discriminate.kind(value) do
      ...
      {:now, value} ->
        waiting = Map.put(waiting, spawned, {:done, value})
  ...
  defp stream_spawn(value, spawned, waiting, config) do
    ...
    receive do
      {:spawned, {^monitor_ref, ^spawned}, pid} ->
        mfa = normalize_mfa(mfa)
        send(pid, {self(), {monitor_ref, spawned}, self(), callers, mfa})
  ...
  defp handle_stop_or_parent_down(
    ...
    for {_ref, %{pid: pid}} <- running_tasks do
